...
class TestQNetwork(unittest.TestCase):
    ...
    def test_layer_init(self):
        net = QNetwork(1, 6, "relu", 4, [2, 3, 4, 5])
        expected = range(1, 7)
        self.assertTrue(len(net.layers) == 5)
        for i, l in enumerate(net.layers):
            self.assertEqual(l.in_features, expected[i])
            self.assertEqual(l.out_features, expected[i + 1])
